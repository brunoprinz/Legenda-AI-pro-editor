    ...
    <div className="min-h-screen bg-brand-950 text-white flex flex-col">
      <header className="h-16 border-b border-brand-800 flex items-center justify-between px-6 bg-brand-900/80 backdrop-blur-md sticky top-0 z-[100]">
        ...
        <div className="flex items-center gap-4 relative z-[110]">
          <select 
            ...
            className="bg-brand-800 border border-brand-700 text-xs p-2 rounded outline-none cursor-pointer text-white"
          >
            <option value="240p">240p (Rápido)</option>
            <option value="480p">480p (Médio)</option>
            <option value="original">Original</option>
          </select>

          <button onClick={() => setShowImportArea(!showImportArea)} className="flex items-center gap-2 px-4 py-2 bg-brand-700 hover:bg-brand-600 rounded-lg text-sm transition cursor-pointer">
            <ClipboardPaste size={18} /> Importar IA
          </button>
          
          <button onClick={handleExport} disabled={!!exportStatus || !videoFile} className={`flex items-center gap-2 px-6 py-2 rounded-lg font-bold transition text-sm cursor-pointer ${exportStatus ? 'bg-brand-800 text-gray-500' : 'bg-brand-500 hover:bg-brand-400 text-white'}`}>
            {exportStatus ? (<><Loader2 className="animate-spin" size={18} /> {Math.round(exportStatus.progress)}%</>) : (<><Download size={18} /> Exportar</>)}
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden">
        <div className="flex-1 flex flex-col p-6 overflow-y-auto">
          {showImportArea && (
            <div className="mb-6 p-6 bg-brand-900 border-2 border-brand-500 rounded-2xl shadow-xl">
              <div className="flex justify-between items-center mb-4">
                <h2 className="font-bold">Assistente IA</h2>
                <button onClick={() => setShowImportArea(false)}>✕</button>
              </div>
              <div className="space-y-4">
                <div className="bg-brand-950 p-3 rounded border border-brand-800">
                  <p className="text-xs mb-2">1. Copie o prompt:</p>
                  <div className="flex gap-2">
                    <code className="flex-1 bg-black p-2 rounded text-[10px] text-brand-400">{promptParaGemini}</code>
                    <button onClick={() => navigator.clipboard.writeText(promptParaGemini)} className="px-2 py-1 bg-brand-700 rounded text-[10px]">Copiar</button>
                  </div>
                  <a href="https://gemini.google.com" target="_blank" rel="noreferrer" className="text-[10px] text-brand-500 mt-2 block hover:underline">2. Abrir Gemini</a>
                </div>
                <textarea className="w-full h-24 bg-brand-950 p-2 rounded border border-brand-700 text-xs text-white" placeholder="3. Cole o JSON aqui..." value={rawJson} onChange={(e) => setRawJson(e.target.value)} />
                <button onClick={handleImportJson} className="w-full py-2 bg-brand-500 rounded font-bold text-sm">4. Gerar Legendas</button>
              </div>
            </div>
          )}
          ...
          {!videoFile ? (
            ...
            <div className="space-y-4">
              <div className="aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl relative">
                ...
                <button onClick={togglePlay} className="absolute bottom-4 left-4 p-3 bg-brand-500 rounded-full hover:scale-110 transition shadow-lg">
